...
let range = ['7d','30d','90d','mtd','all','custom'].includes(params.get('range'))?params.get('range'):'mtd';
let customStart=params.get('from')||'', customEnd=params.get('to')||'';
if(range==='custom'&&(!customStart||!customEnd))range='mtd';
...
let heatYear=range==='custom'?Number(customEnd.slice(0,4)):null, heatScale='ln', selectedHeatDate='', heatLogs=null, heatLoading=false, heatSequence=0;
...
function todayInTimezone(){const parts=new Intl.DateTimeFormat('en-US',{timeZone:tz,year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date());const part=k=>parts.find(p=>p.type===k).value;return `${part('year')}-${part('month')}-${part('day')}`;}
function routeUrl(next=page){const query=new URLSearchParams({range,tz});if(range==='custom'){query.set('from',customStart);query.set('to',customEnd);}if(next==='logs'&&logDate)query.set('date',logDate);return `/${next}?${query}`;}
function setCustomPanel(open){const panel=$('#custom-range-panel');panel.hidden=!open;$('#custom-range-toggle').setAttribute('aria-expanded',String(open));if(open){const today=todayInTimezone();$('#custom-start').max=today;$('#custom-end').max=today;$('#custom-start').value=customStart||data?.start||today;$('#custom-end').value=customEnd||data?.end||today;$('#custom-range-error').textContent='';}}
function navigate(next, push=true){page=next;if(next!=='logs')logDate='';setCustomPanel(false);if(push)history.pushState({},'',routeUrl());load();}
...
function apiUrl(refresh){const query=new URLSearchParams({range,tz});if(range==='custom'){query.set('from',customStart);query.set('to',customEnd);}if(page==='logs'){query.set('page',logPage);query.set('limit',logLimit);query.set('model',logModel);query.set('date',logDate);}if(refresh)query.set('refresh','1');return `/api/${page==='logs'?'logs':'usage'}?${query}`;}
async function load(refresh=false){const id=++sequence;$('#content').setAttribute('aria-busy','true');$('#refresh').disabled=true;try{const res=await fetch(apiUrl(refresh));const result=await res.json();if(!res.ok)throw new Error(result.error||'无法读取数据');if(id!==sequence)return;data=result;chartRows=trendRows(data.daily);render();}catch(e){if(id!==sequence)return;$('#notice').hidden=false;$('#notice').className='notice error';$('#notice').textContent=`读取失败：${e.message}。请确认本地服务正在运行，然后点击刷新重试。`;$('#content').setAttribute('aria-busy','false');}finally{if(id===sequence)$('#refresh').disabled=false;}}
...
document.addEventListener('click',e=>{const p=e.target.closest('[data-page]'),r=e.target.closest('[data-range]'),m=e.target.closest('[data-metric]'),y=e.target.closest('[data-heat-year]'),s=e.target.closest('[data-heat-scale]'),open=e.target.closest('[data-open-day-logs]');if(p){e.preventDefault();navigate(p.dataset.page);}if(r){if(r.dataset.range==='custom'){setCustomPanel($('#custom-range-panel').hidden);}else{logPage=1;logDate='';range=r.dataset.range;setCustomPanel(false);history.replaceState({},'',routeUrl());load();}}if(m){metric=m.dataset.metric;render();}if(y&&y.dataset.heatYear){heatYear=Number(y.dataset.heatYear);selectedHeatDate='';heatLogs=null;render();}if(s){heatScale=s.dataset.heatScale;render();}if(open){page='logs';range='all';logDate=selectedHeatDate;logPage=1;history.pushState({},'',routeUrl());load();}if(e.target.closest('#export'))exportCSV();});
$('#custom-range-panel').onsubmit=e=>{e.preventDefault();const from=$('#custom-start').value,to=$('#custom-end').value,today=todayInTimezone();const error=$('#custom-range-error');if(!from||!to||from>to||to>today){error.textContent=!from||!to?'请选择起止日期。':from>to?'开始日期不能晚于结束日期。':'结束日期不能晚于今天。';return;}customStart=from;customEnd=to;range='custom';heatYear=Number(to.slice(0,4));logPage=1;logDate='';history.replaceState({},'',routeUrl());setCustomPanel(false);load();};
$('#custom-range-cancel').onclick=()=>setCustomPanel(false);
document.addEventListener('pointerdown',e=>{if(!e.target.closest('.range-picker'))setCustomPanel(false);});
document.addEventListener('keydown',e=>{if(e.key==='Escape')setCustomPanel(false);});
$('#refresh').onclick=()=>load(true);
$('#timezone').onchange=e=>{logPage=1;tz=e.target.value;setCustomPanel(false);history.replaceState({},'',routeUrl());load();};
window.onpopstate=()=>{const p=new URLSearchParams(location.search);range=['7d','30d','90d','mtd','all','custom'].includes(p.get('range'))?p.get('range'):'mtd';customStart=p.get('from')||'';customEnd=p.get('to')||'';if(range==='custom'&&(!customStart||!customEnd))range='mtd';tz=p.get('tz')||'Asia/Singapore';logDate=p.get('date')||'';$('#timezone').value=tz;page=['/activity','/logs'].includes(location.pathname)?location.pathname.slice(1):'summary';if(range==='custom')heatYear=Number(customEnd.slice(0,4));setCustomPanel(false);load();};
...
document.addEventListener('change',e=>{if(e.target.id==='log-model'){logModel=e.target.value;logPage=1;load();}if(e.target.id==='log-limit'){logLimit=Number(e.target.value);logPage=1;load();}if(e.target.id==='log-date'){logDate=e.target.value;logPage=1;history.replaceState({},'',routeUrl());load();}if(e.target.id==='heat-year'){heatYear=Number(e.target.value);selectedHeatDate='';heatLogs=null;render();}});
document.addEventListener('click',e=>{const p=e.target.closest('[data-log-page]'),r=e.target.closest('[data-request]'),clear=e.target.closest('[data-clear-log-date]');if(p){logPage=Number(p.dataset.logPage);load();}if(r){const detail=$('#request-'+r.dataset.request);detail.hidden=!detail.hidden;r.setAttribute('aria-expanded',!detail.hidden);}if(clear){logDate='';logPage=1;history.replaceState({},'',routeUrl());load();}});
render();load();
